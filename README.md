...
**Go deep on a posting that resonates.** When one lands and your reaction is *I want this job* — and you feel qualified for it, whether or not your profile currently proves that — it's worth a deep dive. That second half is the actual signal: feeling more qualified than your written record shows means the gap is in the record, and digging closes it. The normal loop is built not to go looking; this one inverts that and works through the posting a requirement at a time, following up until an answer couldn't be misread, writing down what surfaces. Everything it turns up is available to every posting afterward, so **even a rejection leaves you with a bigger profile.** Depending on the size of the posting it can take a day or more, so it isn't something to run on everything you apply to. See [`skills/run-a-deep-dive.md`](skills/run-a-deep-dive.md).
